/* Tanah yang tidak nol: kawat kembali bersama (Rangkaian DC)
 * Sensor (keluaran V_s, arus 10 mA) dan motor (arus I_m berdenyut 1 Hz) memakai kawat kembali yang sama
 * dengan hambatan R_g ke tanah catu daya. Pengukur (ADC) mengacu ke tanah catu, sensor mengacu ke tanah lokal G′.
 * Asumsi model:
...
(function () {
  ...
  SimCore.register({
    api: 1,
    id: 'loop-tanah',
    title: 'Tanah yang tidak nol karena kawat kembali bersama',
    aspect: 16 / 9,
    ...
    graphs: [
      { title: 'Potensial tanah lokal G′', unit: 'V', min: 0, window: 4, series: [{ key: 'VG', label: 'V_G′', color: 'vector' }] },
    ...
    draw(ctx, st, p, v, d) {
      ...
      // Catu daya di kiri, tanah sebenarnya G
      d.source(0, 0, 0, 1, '5 V', false, -1);
      d.ground(0, 0); d.text(0.05, -0.6, 'G (tanah catu)', 'muted', 'sm', 'left');
      d.wire([[0, 1], [2.5, 1]]);
      ...
      // Tanah lokal G′ dan kawat kembali dengan R_g
